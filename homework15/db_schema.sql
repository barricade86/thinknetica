...
create table movies_actors_and_directors(movie_id int not null,director_id int not null,actor_id int not null);
create unique index movies_actors_and_directors_unq on movies_actors_and_directors (movie_id,director_id,actor_id);
CREATE TYPE rating AS ENUM ('PG-10', 'PG-13', 'PG-18');
create table if not exists movies(id serial primary key, title TEXT, year smallint not null, box_office int not null, ratings rating);
create table if not exists actors(id serial primary key, name TEXT not null, birth_date date not null);
create table if not exists directors(id serial primary key, name TEXT not null, birth_date date not null);
create table if not exists studio(id serial primary key, title TEXT);
insert into movies(id, title,year,box_office,ratings) values(1,'Nightmare on the Elm street',1984,25504513,'PG-18');
insert into movies(id, title,year,box_office,ratings) values(2,'Freddy is dead:The final Nightmare',1991,34872033,'PG-18');
insert into movies(id, title,year,box_office,ratings) values(3,'Wishmaster',1997,15738769,'PG-18');
insert into movies(id, title,year,box_office,ratings) values(4,'Wishmaster2: Evil never dies',1999,15000769,'PG-18');
insert into movies(id, title,year,box_office,ratings) values(5,'DragonHeart',1996,51367375,'PG-18');
insert into movies(id, title,year,box_office,ratings) values(6,'A Nightmare on Elm Street Part 2: Freddy''s Revenge',1985,29999213,'PG-18');
